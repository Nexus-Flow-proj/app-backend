...
export class StorageService {
  ...
  constructor(private readonly configService: ConfigService) {
    const supabaseUrl =
      this.configService.get<string>('env.supabaseUrl') ||
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      this.logger.error(
        'Supabase URL or Service Role Key is missing in configuration.',
      );
    }
  ...
  async deleteAvatar(avatarUrl: string): Promise<void> {
    ...
    try {
      const storagePath = this.extractPathFromUrl(
        avatarUrl,
        this.AVATARS_BUCKET,
      );
      ...
      if (error) {
        this.logger.warn(
          `Failed to delete avatar (${storagePath}): ${error.message}`,
        );
      }
    } catch (err) {
  ...
  async uploadAttachment(
    ...
    if (error) {
      this.logger.error(
        `Failed to upload attachment for task ${taskId}: ${error.message}`,
      );
      throw new InternalServerErrorException(
  ...
  /**
   * Upload single chat attachment to 'attachments' bucket under chat/<projectId>/...
   */
  async uploadChatAttachment(
    ...
    const sanitized = sanitizeFileName(originalName);
    const filePath = `chat/${projectId}/${Date.now()}-${sanitized}`;
    ...
    if (error) {
      this.logger.error(
        `Failed to upload chat attachment for project ${projectId}: ${error.message}`,
      );
      throw new InternalServerErrorException(
        `Chat attachment upload failed: ${error.message}`,
      );
    }
  ...
  async deleteAttachment(pathOrUrl: string): Promise<void> {
    ...
    try {
      ...
      if (pathOrUrl.startsWith('http://') || pathOrUrl.startsWith('https://')) {
        const extracted = this.extractPathFromUrl(
          pathOrUrl,
          this.ATTACHMENTS_BUCKET,
        );
      ...
      if (error) {
        this.logger.warn(
          `Failed to delete attachment (${storagePath}): ${error.message}`,
        );
      }
    } catch (err) {
